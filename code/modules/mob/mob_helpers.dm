...
/proc/stars(n, pr)
	n = sanitize_local(n, SANITIZE_TEMP)
	if (pr == null)
		pr = 25
	if (pr <= 0)
		return null
	...
	while(p <= n)
		...
		if (char == ">")
			intag = !intag
		p++
	return sanitize_local(t, SANITIZE_CHAT)

/proc/slur(phrase)
	phrase = lhtml_decode(phrase, SANITIZE_TEMP)
	...
	while(counter>=1)
		...
		if(rand(1,3)==3)
			if(lowerrustext(newletter)=="о")	newletter="а"
			if(lowerrustext(newletter)=="с")	newletter="щ"
			if(lowerrustext(newletter)=="а")	newletter="ах"
		switch(rand(1,15))
			if(1,3,5,8)	newletter="[sanitize_local(lowerrustext(newletter), SANITIZE_TEMP)]"
	...
	return sanitize_local(newphrase)

/proc/stutter(n)
	var/te = lhtml_decode(n, SANITIZE_TEMP)
	...
	while(p <= n)//while P, which starts at 1 is less or equal to N which is the length.
		...
		if (prob(40) && (n_letter in list("b","c","d","f","g","h","j","k","l","m","n","p","q","r","s","t","v","w","x","y","z","б","в","г","д","е","ж","з","и","к","л","м","н","п","р","с","т","ф","х","ц","ч","ш","щ")))
			if (prob(10))
